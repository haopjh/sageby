...
@interface RadioTemplateViewController ()

@end

@implementation RadioTemplateViewController
...
- (void)viewDidLoad
{
    ...
    //qLabel.text = [NSString stringWithFormat:@"%@\n\n", question];
    qLabel.text = [NSString stringWithFormat:@"%@", question];
    //Question Label Autosizing
    //[qLabel sizeToFit];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if([self.arForIPs containsObject:indexPath]){
        UIImageView *image = [[UIImageView alloc] initWithImage:[UIImage imageNamed:@"RadioChecked.png"]];
        [cell setAccessoryView:image];
	} else {
        UIImageView *image = [[UIImageView alloc] initWithImage:[UIImage imageNamed:@"RadioUnchecked.png"]];
        [cell setAccessoryView:image];
	}
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
	...
	if([self.arForIPs containsObject:indexPath]){
        ...
        if([self.arForIPs count] == 0) {
            [self.arForIPs addObject:indexPath];
        } else {
            [self.arForIPs removeAllObjects];
            [self.arForIPs addObject:indexPath];  
        }
	}
    ...
    if (![surveyQuestion optional]) {
        if ([self.arForIPs count] != 1) {
            ...
            UIAlertView *av = [[UIAlertView alloc] initWithTitle:@"Incomplete!" 
                                                         message:@"Please select only one option"delegate:nil cancelButtonTitle:@"OK" otherButtonTitles:nil];
            [av show];
            return NO;
        }
