...
//    oneDimBarcode = [result retain];
    [self setOneDimBarcode:nil];
    [self setOneDimBarcode:result];
}
...
-(void)setupQRCode:(NSString *)code
{
    ...
    CQR_Encode encoder;
    encoder.EncodeData(0, 0, true, -1, str);
...
//    qRBarcode = [[UIImage imageWithCGImage:imageRef] retain]; 
    [self setQRBarcode:[UIImage imageWithCGImage:imageRef]];
