...
- (void)viewDidLoad {
	...
	//self.emailField.placeholder = ATLocalizedString(@"Your Email Address", @"Email Address Field Placeholder");
	
	NSUserDefaults *defaults = [NSUserDefaults standardUserDefaults];
	//NSLog(@"This is user's email %@",[defaults objectForKey:@"SagebyUserEmail"]);
	self.emailField.text = (NSString *)[defaults objectForKey:@"SagebyUserEmail"];
...
- (IBAction)donePressed:(id)sender {
	...
	if (self.showEmailAddressField && (!self.feedback.email || [self.feedback.email length] == 0)) {
		...
		field.placeholder = NSLocalizedString(@"Your Email Address", @"Email address popup placeholder text.");
