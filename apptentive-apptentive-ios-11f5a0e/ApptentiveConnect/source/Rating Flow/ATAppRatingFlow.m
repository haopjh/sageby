...
#endif
{
	NSString *title = [NSString stringWithFormat:ATLocalizedString(@"Do you like %@?", @"Title for enjoyment alert view. Parameter is app name."), [self appName]];
#if TARGET_OS_IPHONE
	self.viewController = vc;
	if (!enjoymentDialog) {
...
#endif
{
	...
	NSString *message = [NSString stringWithFormat:ATLocalizedString(@"We're so happy to hear that you like %@! It'd be really helpful if you rated us in the App Store. Thanks so much for spending some time with us.", @"Rate app message. Parameter is app name."), [self appName]];
	...
	if (alertView == enjoymentDialog) {
		...
		if (buttonIndex == 0) { // no
			...
			if (!self.viewController) {
				...
				connection.customPlaceholderText = ATLocalizedString(@"What can we do to ensure that you like our app? We appreciate your constructive feedback.", @"Custom placeholder feedback text when user is unhappy with the application.");
